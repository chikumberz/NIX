<?php
	...
	class File extends \Phalcon\Mvc\User\Component {
		...
		public static function mode ( $source, $mode = 755 ) {

			if ( self::exists( $source ) )
				return chmod( $source, $mode );
		...
		public static function writable ( $source ) {
			...
			if ( !is_file( $source ) || ( $fp = @fopen( $source, 'ab' ) ) === false ) {
				return false;
			}
		...
		public static function translate ( $source, $convert_spaces = false, $replace_with = '_' ) {

			if ( $convert_spaces ) {
				$source = str_replace( ' ', $replace_with, $source );
			}

			if ( function_exists( 'transliterator_transliterate' ) ) {
				$source = transliterator_transliterate( 'Accents-Any', utf8_encode( $source ) );
			} else {
		...
		public static function mimeType ( $source ) {
			...
			if ( !function_exists( 'get_file_mime_type' ) ) {
				function get_file_mime_type( $filename, $debug = false ) {
					...
					if ( !empty( $mime_types[ $extension ] ) ) {
						if ( true === $debug ) {
							return array( 'mime_type' => $mime_types[$extension], 'method' => 'from_array' );
						}
